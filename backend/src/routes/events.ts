...
import { registerForEvent, getEventAttendees } from "../controllers/ticketController";
import {
  submitFeedback,
  getMyFeedback,
  getEventFeedback,
} from "../controllers/feedbackController";
import { getEventNetworking } from "../controllers/networkingController";
import { protect, optionalAuth, authorize, requireRole, requirePermission } from "../middleware/auth";
...
router.post(
  ...
  [
    ...
    body("date")
      .if((value: any, { req }: any) => req.body.status !== "Draft")
      .notEmpty().withMessage("Date is required")
      .isISO8601().withMessage("Invalid date"),
    body("date").if((value: any, { req }: any) => req.body.status === "Draft").optional({ checkFalsy: true }).isISO8601().withMessage("Invalid date"),
    body("venue")
      .if((value: any, { req }: any) => req.body.status !== "Draft")
      .notEmpty().withMessage("Venue is required"),
    body("venue").if((value: any, { req }: any) => req.body.status === "Draft").optional({ checkFalsy: true }).isString(),
    body("category").notEmpty().withMessage("Category is required"),
    body("capacity")
      .if((value: any, { req }: any) => req.body.status !== "Draft")
      .isInt({ min: 1, max: 1000000 }).withMessage("Capacity must be between 1 and 1,000,000"),
    body("capacity").if((value: any, { req }: any) => req.body.status === "Draft").optional({ checkFalsy: true }).isInt({ min: 1, max: 1000000 }).withMessage("Capacity must be between 1 and 1,000,000"),
    body("type")
      .optional()
...
router.put(
  ...
  [
    ...
    body("price").optional().custom((val: any) => {
