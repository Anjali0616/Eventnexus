...
import { parsePagination, buildSearch, buildAdvancedFilters, parseSort, paginate } from "../utils/query";
import { notifyNearbyUsers } from "../utils/proximityNotify";
...
export const createEvent = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (status && !EVENT_STATUSES.includes(status)) {
      return void res.status(400).json({ message: `Invalid status — must be one of: ${EVENT_STATUSES.join(", ")}` });
    }
    ...
    if (date) {
      ...
      if (isNaN(eventDate.getTime())) {
        return void res.status(400).json({ message: "Invalid event date" });
      }
      if (!isDraft) {
        if (eventDate <= new Date()) {
          return void res.status(400).json({ message: "Event date must be in the future" });
        }
        if (eventDate.getTime() - Date.now() > MAX_FUTURE_EVENT_MS) {
          return void res.status(400).json({ message: "Event date can't be more than 2 years in the future" });
        }
      } else if (status === "Past" && eventDate > new Date()) {
        return void res.status(400).json({ message: "Past event must have a past date" });
      }
    } else if (!isDraft) {
      return void res.status(400).json({ message: "Event date is required" });
    }
    const cap = Number(capacity);
    if (!isDraft) {
      if (!Number.isInteger(cap) || cap < 1) {
        return void res.status(400).json({ message: "Capacity must be a positive integer" });
      }
      if (cap > 1_000_000) {
        return void res.status(400).json({ message: "Capacity can't exceed 1,000,000" });
      }
      if (!venue || venue.trim().length < 2) {
        return void res.status(400).json({ message: "Venue is required" });
      }
    ...
    if (normalizedPrice.amount < 0) {
      return void res.status(400).json({ message: "Price can't be negative" });
    }
    ...
    if (!req.user.organization) {
      return void res.status(403).json({ message: "System admins cannot create events — assign an organization or use an org_admin/organizer account" });
    }
...
export const getOrgEvents = async (req: Request, res: Response): Promise<void> => {
  try {
    const { page, limit, skip } = parsePagination(req.query, { defaultLimit: 9 });
...
export const listCoHostOrganizations = async (req: Request, res: Response): Promise<void> => {
  try {
    const event = await Event.findById(req.params.id)
...
export const removeCoHostOrganization = async (req: Request, res: Response): Promise<void> => {
  try {
    const event = await Event.findById(req.params.id);
    if (!event) {
      return void res.status(404).json({ message: "Event not found" });
    ...
    if (!event.coHostOrganizations?.some((oid) => oid.toString() === orgId)) {
      return void res.status(404).json({ message: "Co-host organization not found" });
    }
...
export const getEventAiInsight = async (req: Request, res: Response): Promise<void> => {
  try {
    const event = await Event.findById(req.params.id);
    if (!event) {
      return void res.status(404).json({ message: "Event not found" });
...
export const generateEventDraft = async (req: Request, res: Response): Promise<void> => {
  try {
    const { title, category, type, venue, capacity } = req.body;
    if (!title || title.trim().length < 3) {
      return void res.status(400).json({ message: "Title is required (min 3 chars) to generate draft" });
    }
...
const _controllerExports = { createEvent, getMyEvents, getEventById, updateEvent, deleteEvent, getAllEvents, getOrgEvents, getEventAiInsight, generateEventDraft, removeCoHostOrganization, listCoHostOrganizations, canManageEvent };
