...
import { slugify } from "./organizationController";
import { audit } from "../utils/audit";
import { sendMail } from "../utils/email";

import { generateRefreshToken, generateEmailToken, hashToken, parseDuration } from "../utils/tokens";
import { parsePagination, parseSort, paginate } from "../utils/query";
...
export const register = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (resolvedRole === "org_admin") {
      ...
      if (!organizationId) {
        return void res.status(400).json({ message: "Organization is required for organizer" });
      }
      organization = await Organization.findById(organizationId);
      if (!organization || organization.status !== "active") {
        return void res.status(400).json({ message: "Invalid organization" });
      }
    } else {
...
export const orgRegister = async (req: Request, res: Response): Promise<void> => {
  try {
    const {
      orgName,
      orgEmail,
    ...
    if (existingUser) {
      return void res.status(400).json({ message: "An account with that email already exists" });
    }
    const slug = slugify(orgName);
    if (await Organization.findOne({ slug })) {
      return void res.status(400).json({ message: "An organization with that name already exists" });
    }
...
export const login = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (disabled) {
      return void res.status(disabled.status).json({ message: disabled.message });
    }

    const gate = await assertOrgApproved(user);
    if (gate) {
      return void res.status(gate.status).json({ message: gate.message, code: gate.code });
    }
...
export const googleLogin = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!credential) {
      return void res.status(400).json({ message: "Missing Google credential" });
    }
...
export const refresh = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!refreshToken) {
      return void res.status(400).json({ message: "refreshToken is required" });
    }
    ...
    if (session.revokedAt || session.expiresAt <= new Date()) {
      return void res.status(401).json({ message: "Refresh token has expired or was revoked" });
    }
...
export const listSessions = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const filter: any = { user: req.user._id, revokedAt: null };
...
export const revokeSession = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!session) {
      return void res.status(404).json({ message: "Session not found" });
    }
    session.revokedAt = new Date();
    await session.save();
    audit({
...
export const verifyEmail = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!token) {
      return void res.status(400).json({ message: "Verification token is required" });
    }
    const user = await User.findOne({ emailVerificationToken: hashToken(token) }).select(
      "+emailVerificationToken"
    );
    if (!user || !user.emailVerificationExpiresAt || user.emailVerificationExpiresAt < new Date()) {
      return void res.status(400).json({ message: "Verification link is invalid or has expired" });
    }
    user.emailVerifiedAt = new Date();
...
export const resendVerification = async (req: Request, res: Response): Promise<void> => {
  try {
    if (req.user.emailVerifiedAt) {
      return void res.status(400).json({ message: "Email is already verified" });
    }
...
const _controllerExports = { register, orgRegister, login, googleLogin, getMe, refresh, logout, listSessions, revokeSession, verifyEmail, resendVerification, forgotPassword, resetPassword, exportMyData, deleteMyAccount };
