...
import * as predictAttendance from "./predictAttendance";
...
export const EXPLICIT_INTEREST_BOOST: number = 28;

export const proximityScore = (distanceKm: number | null | undefined): number => {
...
export const describeFactors = ({ fillRate, velocityScore, proximity, recencyScore, categoryMatched, typeMatched, comboMatched, explicitInterestMatched }: {
  fillRate: number;
  ...
  explicitInterestMatched?: boolean;
}): string[] => {
...
export const fallbackReason = (s: any): string => {
...
export const addAiReasons = async (scored: any[]): Promise<void> => {
  ...
  const reply: string | null = await ai.generate(systemPrompt, promptLines.join("\n"));
...
export const scoreDeterministic = async ({
  candidates,
  myTickets,
  location,
  limit,
  ...
  categoryLastSeen: Record<string, Date>;
  registeredEventIds: Set<string>;
  userInterests?: string[];
}): Promise<{ scored: any[]; hasLocation: boolean }> => {
  const hasUserLoc = hasValidCoords(location);
  const interestSet = new Set(userInterests || []);

  const scored = candidates
    .filter((event: any) => !registeredEventIds.has(event._id.toString()))
    ...
    .filter((event: any) => event.registered < event.capacity)
    .map((event: any) => {
      ...
      const predicted: number = (predictAttendance as any).heuristic(event);
      ...
      const distanceKm: number | null =
        hasUserLoc && hasValidCoords(event.coordinates)
          ? Math.round((haversineKm(location, event.coordinates) as number) * 10) / 10
          : null;
      const proximity = proximityScore(distanceKm);

      let recencyScore = 0;
      const lastSeen: Date | undefined = categoryLastSeen[event.category];
      if (lastSeen) {
...
export const buildFromCf = async ({ candidates, cf, location, limit, registeredEventIds, userInterests = [] }: {
  candidates: any[];
  cf: any;
  location: any;
  limit: number;
  registeredEventIds: Set<string>;
  userInterests?: string[];
}): Promise<{ scored: any[]; hasLocation: boolean } | null> => {
  const hasUserLoc = hasValidCoords(location);
  const interestSet = new Set(userInterests || []);
  const byId = new Map(candidates.map((e: any) => [e._id.toString(), e]));
  const entries: any[] = [];
  for (const r of cf.recommendations) {
    const event: any = byId.get(String(r.event_id));
  ...
  const predictions: number[] = await (predictAttendance as any).batch(entries.map((e: any) => e.event));

  const scored = entries.map(({ event, cfScore }: any, i: number) => {
    const explicitInterestMatched = interestSet.has(event.category);
...
export const scoreEvents = async ({ attendee, organization, location, limit = 12, withReasons = false, userInterests }: {
  attendee: any;
  organization?: any;
  location?: any;
  limit?: number;
  withReasons?: boolean;
  userInterests?: string[];
}): Promise<{ scored: any[]; hasLocation: boolean; recommendations: any[]; interests: string[] }> => {
  const [myTickets, candidates, userDoc]: [any[], any[], any] = await Promise.all([
    (Ticket as any).find({ attendee }).populate("event").sort({ createdAt: -1 }).lean(),
    ...
    userInterests !== undefined
      ? null
      : (User as any).findById(attendee).select("interests").lean(),
  ]);
  const resolvedInterests: string[] = userInterests !== undefined
    ? userInterests
    : (userDoc?.interests || []);

  const registeredEventIds = new Set(
    myTickets.filter((t: any) => t.event).map((t: any) => t.event._id.toString())
  );

  // Tier 1: collaborative filtering from the Python AI service.
  const cf: any = await ai.recommend(attendee, organization);
  ...
  const categoryLastSeen: Record<string, Date> = {};

  myTickets.forEach((t: any) => {
    if (!t.event) return;
    const cat: string = t.event.category;
    const typ: string = t.event.type;
    const key = `${cat}|${typ}`;
  ...
  const predictions: number[] = await (predictAttendance as any).batch(scored.map((s: any) => s.event));
  scored.forEach((s: any, i: number) => (s.predicted = predictions[i]));
...
export default { scoreEvents, proximityScore };
